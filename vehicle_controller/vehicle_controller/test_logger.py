...
stack and the one whose bandwidth actually matters for later ISO 2631-1
post-processing. Slower topics (ground truth pose/twist, MPC debug,
final command) are cached and stamped onto each IMU-driven row.
"""
import csv
import os
import time
...
RESULT_NAMES = {0: 'RUNNING', 1: 'SUCCESS', 2: 'ROLLOVER', 3: 'STUCK', 4: 'FALL'}
...
class TestLogger(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('output_dir', os.path.expanduser('~/ppo_test_logs'))
        ...
        self.get_logger().info(f'test_logger ready — writing to {self.output_dir}')
    ...
    def _result_cb(self, msg: Int32):
        ...
        path = os.path.join(
            self.output_dir,
            f'{self.run_id}_seed{self.seed}_{traj_tag}_{result_name}_{int(time.time())}.csv'
        )
